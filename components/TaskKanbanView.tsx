...
export const TaskKanbanView: React.FC<TaskKanbanViewProps> = ({
  ...
  if (isLoading) {
    return (
      <div className="flex-1 overflow-x-auto p-4 md:p-6 w-full h-full min-h-0">
        <div className="flex gap-4 min-w-[1100px] h-full items-start">
          {Array.from({ length: 5 }).map((_, colIdx) => (
            <div
              key={colIdx}
              className="flex-1 bg-muted/20 border border-border rounded-md p-3 space-y-3 min-w-[210px] h-full flex flex-col"
            >
              ...
              <div className="space-y-2.5 overflow-y-auto flex-1 p-1">
  ...
  return (
    <div className="flex-1 pl-2 pr-4 md:py-3 flex flex-col bg-background min-h-0 overflow-hidden h-full">
      {/* View Header */}
      <div className="flex items-center justify-between pb-2.5 mb-2.5 flex-wrap gap-2 border-b border-border shrink-0">
        <div>
      ...
      <div className="flex-1 min-h-0 overflow-x-auto overflow-y-hidden pb-2 pr-1">
        <div className="flex gap-4 min-w-[1400px] h-full items-stretch pb-1">
          {columns.map((col) => {
            ...
            return (
              <div
                ...
                className={`w-72 md:w-80 bg-card border rounded-md flex flex-col shrink-0 h-full min-h-0 transition-all duration-200 shadow-xs ${
                  isOver
              ...
              >
                {/* Column Header */}
                <div
                  className="p-3 border-b border-border bg-card flex items-center justify-between shrink-0 rounded-t-md select-none"
                ...
                {/* Column Body / Droppable Area with scrollable content & bottom padding */}
                <div className="p-2.5 pb-8 space-y-2.5 overflow-y-auto flex-1 min-h-0 overscroll-contain">
                  {colTasks.length === 0 ? (
                    <div
                  ...
                  {/* Extra bottom spacer to ensure the last card is always 100% visible and unclipped when scrolled */}
